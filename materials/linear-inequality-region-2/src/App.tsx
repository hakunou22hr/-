...
function Graph({active,k,showCommon,showLine,onVertex}:{active:Inequality[],k:number,showCommon:boolean,showLine:boolean,onVertex:(i:number)=>void}){
 ...
 const hit=vertices.findIndex(v=>Math.abs(2*v.x+v.y-k)<=.06)
 const objective:Inequality={id:'k',label:'',boundary:'',a:2,b:1,c:k,color:'#d62f52'}
 return <div className="graph-wrap"><svg className="graph" viewBox={`0 0 ${SIZE} ${SIZE}`} role="img" aria-label="選択した連立不等式の座標平面">
  ...
  {showLine&&<text className="line-label" x={sx(Math.max(MIN,Math.min(MAX,k/3)))} y={sy(Math.max(MIN,Math.min(MAX,k-2*Math.max(MIN,Math.min(MAX,k/3)))))-13}>2x + y = {formatNumber(k)}</text>}
 ...
 </svg>{hit>=0&&<div className="hit-card"><small>頂点に到達！</small><strong>（{formatNumber(vertices[hit].x)}，{formatNumber(vertices[hit].y)}）</strong><span>2x + y = 2×{formatNumber(vertices[hit].x)} + {formatNumber(vertices[hit].y)} = {formatNumber(2*vertices[hit].x+vertices[hit].y)}</span><b>k = {formatNumber(2*vertices[hit].x+vertices[hit].y)}</b></div>}</div>
}
...
export default function App(){
 ...
 return <div className="app"><header><a href="../../">← 教材一覧</a><div><p>MATHEMATICS II · INTERACTIVE LESSON</p><h1>連立不等式の領域と <span>最大・最小②</span></h1></div><button className="reset" onClick={reset}><RotateCcw/>リセット</button></header>
  <nav className="modes" aria-label="表示モード">{['領域を見る','共通領域を見る','2x+y=kを動かす','最大・最小を考える'].map((x,i)=><button key={x} className={mode===i+1?'on':''} onClick={()=>setMode(i+1)}><i>{i+1}</i>{x}</button>)}</nav>
  <main><section className="control-panel"><div className="section-title"><span>01</span><div><small>INEQUALITIES</small><h2>不等式を選ぼう</h2></div></div><p className="guide">ボタンを押すと、その式を満たす側が光ります。複数選んで共通部分を探そう。</p><div className="choices">{INEQUALITIES.map(q=>{const on=ids.includes(q.id);return <button key={q.id} aria-pressed={on} className={on?'on':''} style={{'--c':q.color} as React.CSSProperties} onClick={()=>toggle(q.id)}><span>{on?<Check/>:null}</span><b>{q.label}</b><small>境界：{q.boundary}</small></button>})}</div>
   ...
   {mode>=3&&<div className="slider"><div><label htmlFor="k">2x + y = k</label><output>現在の k = <b>{formatNumber(k)}</b></output></div><div className="motion-toggle" aria-label="kの移動方法"><button className={!auto?'on':''} aria-pressed={!auto} onClick={()=>setAuto(false)}><Pause/>手動</button><button className={auto?'on':''} aria-pressed={auto} onClick={()=>setAuto(true)}><Play/>自動</button></div><input id="k" type="range" min="-4" max="18" step="0.1" value={k} disabled={auto} onChange={e=>setK(Number(e.target.value))}/><div className="range"><span>−4</span><span>{auto?'自動で往復中…':'小さく ← 平行移動 → 大きく'}</span><span>18</span></div></div>}
   {vertexInfo&&<div className="vertex-info"><small>選んだ頂点</small><b>点{String.fromCharCode(65+selected!)}　（{formatNumber(vertexInfo.x)}，{formatNumber(vertexInfo.y)}）</b><span>2x + y = 2×{formatNumber(vertexInfo.x)} + {formatNumber(vertexInfo.y)} = <strong>{formatNumber(2*vertexInfo.x+vertexInfo.y)}</strong></span></div>}
  </section>
  <section className="graph-panel"><div className="graph-heading"><div><span>02</span><small>COORDINATE PLANE</small><h2>{active.length?`${active.length}つの不等式を表示中`:'不等式を選択してください'}</h2></div><div className="key"><i/>共通領域 <em/>2x+y=k</div></div><Graph active={active} k={k} showCommon={mode>=2} showLine={mode>=3} onVertex={setSelected}/></section></main>
  {mode>=4&&<section className="thinking"><div className="think-head"><Lightbulb/><div><small>DISCOVERY STEPS</small><h2>考えてみよう</h2></div></div><ol><li><b>問い1</b>2x + y = k の直線を動かしてみよう。kを大きくすると、直線はどちらへ動くだろう？</li><li><b>問い2</b>共通領域と交わる範囲で、kをできるだけ大きくしてみよう。</li><li><b>問い3</b>最後に直線が共通領域と触れるのはどの頂点だろう？</li><li><b>問い4</b>その頂点の座標を使って、2x + y の値を求めよう。</li><li><b>問い5</b>逆にkを小さくすると、最小値はどの頂点で決まるだろう？</li></ol><button className="answer-button" disabled={!result} onClick={()=>setAnswers(v=>!v)}><Eye/>答えを{answers?'隠す':'確認'}</button>{!result&&<p className="answer-hint">4つの不等式を選ぶと、最大・最小を確認できます。</p>}{answers&&result&&<div className="answers"><Answer title="最大値" p={result.max} vertices={vertices}/><Answer title="最小値" p={result.min} vertices={vertices}/></div>}</section>}
 </div>
}
function Answer({title,p,vertices}:{title:string,p:Point,vertices:Point[]}){const i=vertices.indexOf(p);return <div><small>{title}</small><h3>{title}となる点：点{String.fromCharCode(65+i)}（{formatNumber(p.x)}，{formatNumber(p.y)}）</h3><p>2x + y = 2×{formatNumber(p.x)} + {formatNumber(p.y)} = <strong>{formatNumber(2*p.x+p.y)}</strong></p><b>そのとき k = {formatNumber(2*p.x+p.y)}</b></div>}
