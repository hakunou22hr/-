...
export default function App() {
  ...
  const [emphasizeThirdDie,setEmphasizeThirdDie] = useState(true)
  ...
  const reset = () => { setSumMode('normal');setOrdered(false);setShowAll(true);setEmphasizeThirdDie(true);setItem(0);setView('angle');setResetKey(k=>k+1) }
  return <main className="lesson">
    ...
    <section className="workspace">
      <div className="visual">
        <div className="canvas-wrap"><DiceScene ordered={ordered} sums={sums} current={current} showAll={showAll} emphasizeThirdDie={emphasizeThirdDie} view={view} resetKey={resetKey}/><div className="legend"><span><i className="dot cold-bg"/>和が7</span><span><i className="dot warm-bg"/>和が14</span><span><i className="dot third-bg"/>3つ目のさいころ</span></div></div>
        <div className="controls">
          ...
          <div><label>強調</label><button className={emphasizeThirdDie?'on danger':''} aria-pressed={emphasizeThirdDie} onClick={()=>setEmphasizeThirdDie(value=>!value)}>3つ目のさいころを強調 {emphasizeThirdDie?'ON':'OFF'}</button></div>
      ...
      <aside>
        ...
        {(step===4||step===5||sumMode==='7'||sumMode==='14') && <section className={`case-card sum-${focusedSum}`}><div className="case-head"><span>和が{focusedSum}</span><b>{showAll?'4つすべて':`${item+1} / 4`}</b></div><ul>{combinations.map((t,i)=><li className={showAll||i===item?'found':''} key={formatTriple(t)}><button onClick={()=>{setItem(i);setShowAll(false)}}><span>{formatTriple(t)}</span><small>{t.join(' + ')} = {focusedSum}</small></button></li>)}</ul><div className="dice-breakdown" aria-live="polite"><span>1個目 = <b>{combinations[item][0]}</b></span><span>2個目 = <b>{combinations[item][1]}</b></span><strong>3個目 = <b>{combinations[item][2]}</b><small>← 高さ z の位置</small></strong></div><p>和が{focusedSum}になる場合は <strong>4通り</strong></p>{!showAll&&<div className="case-nav"><button onClick={()=>setItem((item+3)%4)}>← 前へ</button><button onClick={()=>setItem((item+1)%4)}>次へ →</button></div>}</section>}
