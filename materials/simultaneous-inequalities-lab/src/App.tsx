...
function Endpoint({value,inclusive,x,y,color,label}:{value:number,inclusive:boolean,x:(v:number)=>number,y:number,color:string,label:string}){return <g transform={`translate(${x(value)} ${y})`} className="endpoint-position"><g className={'endpoint '+(inclusive?'closed':'open')} aria-label={`${value}を${inclusive?'含む':'含まない'}`}><circle className="endpoint-halo" r="18" fill="none" stroke={color}/><circle className="endpoint-dot" r="12" fill={inclusive?color:'#07111f'} stroke={color} strokeWidth="5"/><text className="boundary-label" x="0" y="30" textAnchor="middle" fill={color}>{label}</text></g></g>}
function InequalityRange({q,x,y,color,index,lift}:{q:Inequality,x:(v:number)=>number,y:number,color:string,index:number,lift:'diagonal'|'vertical'}){const v=inequalityVisual(q),edge=v.direction==='right'?x(1e4):x(-1e4),baseX=x(v.boundary),offset=lift==='diagonal'?(v.direction==='right'?18:-18):0,boundary=baseX+offset,axisY=304;return <g className={`inequality-visual direction-${v.direction}`}>
 <line x1={baseX} x2={boundary} y1={axisY} y2={y} stroke={color} className="lift-line" pathLength="1"/>
 {v.direction!=='point'&&<circle cx={boundary} cy={y} r="5" fill={color} className="lift-joint"/>}
 ...
 <Endpoint value={v.boundary} inclusive={v.endpoint==='closed'} x={x} y={axisY} color={color} label={String(v.boundary)}/>
 <text x="22" y={y-28} fill={color} className="row-title">[{index}] {formatInequality(q)}</text>
 </g>}
function NumberLine({items,step,lift}:{items:Inequality[],step:number,lift:'diagonal'|'vertical'}){
 ...
 return <div className="chart"><svg viewBox={`0 0 ${W} 360`} role="img" aria-label="x軸上の境界点から持ち上がる3段の数直線">
  ...
  {[68,148,228].map(y=><line key={y} x1={pad} x2={W-pad} y1={y} y2={y} className="guide-rail"/>)}
  <line x1={pad} x2={W-pad} y1="304" y2="304" className="base axis-main"/>
  <g className={'range-row '+(step>=0?'show':'hide')} filter="url(#redGlow)"><InequalityRange q={items[0]} x={v=>scale(Math.max(min,Math.min(max,v)))} y={68} color="#ff4d61" index={1} lift={lift}/></g>
  <g className={'range-row '+(step>=1?'show':'hide')} filter="url(#greenGlow)"><InequalityRange q={items[1]} x={v=>scale(Math.max(min,Math.min(max,v)))} y={148} color="#42ef9b" index={2} lift={lift}/></g>
  <g className={'range-row common-row '+(step>=2?'show':'hide')} filter="url(#goldGlow)"><text x="22" y="200" fill="#ffd75a" className="row-title">[3] 共通部分</text>
   {!sol.empty&&!sol.point&&(()=>{const a=scale(sol.lower?.value??min),b=scale(sol.upper?.value??max),d=lift==='diagonal'?14:0;return <><path d={`M ${a} 304 L ${a+d} 228 L ${b-d} 228 L ${b} 304`} className="common-platform"/><line x1={a+d} x2={b-d} y1="228" y2="228" stroke="#ffd75a" className="range common-range" pathLength="1"/></>})()}
   {!sol.empty&&sol.lower&&<Endpoint value={sol.lower.value} inclusive={sol.lower.inclusive} x={scale} y={304} color="#ffd75a" label={String(sol.lower.value)}/>}
   {!sol.empty&&sol.upper&&(!sol.lower||sol.upper.value!==sol.lower.value)&&<Endpoint value={sol.upper.value} inclusive={sol.upper.inclusive} x={scale} y={304} color="#ffd75a" label={String(sol.upper.value)}/>}
   {!sol.empty&&sol.point&&<line x1={scale(sol.lower!.value)} x2={scale(sol.lower!.value)} y1="304" y2="228" stroke="#ffd75a" className="lift-line"/>}
   {sol.empty&&<><text x={W/2} y="224" textAnchor="middle" className="empty">共通部分なし</text><text x={W/2} y="246" textAnchor="middle" className="no-solution">解なし</text></>}
  </g>
  {Array.from({length:Math.floor(max)-Math.ceil(min)+1},(_,i)=>i+Math.ceil(min)).map(v=><g key={v}><line x1={scale(v)} x2={scale(v)} y1="299" y2="309" className="tick"/><text x={scale(v)} y="350" textAnchor="middle" className="num minor-number">{v}</text></g>)}<text x={W-pad+12} y="309" className="num">x</text>
 </svg></div>
}
...
function Bar({q,y,color}:{q:Inequality,y:number,color:string}){const v=inequalityVisual(q),start=v.direction==='right'?q.value:-6,end=v.direction==='left'?q.value:6,ground=-2;return <group>{v.direction!=='point'&&<mesh position={[(start+end)/2,y,0]}><boxGeometry args={[Math.max(.08,end-start),.22,1.15]}/><meshStandardMaterial color={color} emissive={color} emissiveIntensity={.45} transparent opacity={.72}/></mesh>}<Line points={[[q.value,ground,0],[q.value,y,0]]} color={color} lineWidth={3}/><Endpoint3D q={q} y={ground} color={color}/></group>}
function Scene3D({items,preset,zoom}:{items:Inequality[],preset:string,zoom:number}){const s=solve(items);let start=s.lower?.value??-6,end=s.upper?.value??6;const lower=s.lower?{op:(s.lower.inclusive?'>=':'>') as Inequality['op'],value:s.lower.value}:null,upper=s.upper?{op:(s.upper.inclusive?'<=':'<') as Inequality['op'],value:s.upper.value}:null;return <Canvas camera={{position:[8,6,9],fov:42}}><color attach="background" args={['#07101c']}/><fog attach="fog" args={['#07101c',12,24]}/><ambientLight intensity={.65}/><pointLight position={[1,6,3]} intensity={30} color="#ffe6a0"/><gridHelper args={[14,14,'#25486b','#142a40']} position={[0,-2,0]}/><Line points={[[-6,-2,0],[6,-2,0]]} color="#7a9bb6" lineWidth={2}/><Bar q={items[0]} y={2} color="#ff4057"/><Bar q={items[1]} y={.4} color="#32e991"/>{!s.empty&&<group><mesh position={[(start+end)/2,-.8,0]}><boxGeometry args={[s.point?.08:Math.max(.08,end-start),.34,1.4]}/><meshStandardMaterial color="#ffd449" emissive="#ffb300" emissiveIntensity={1.4} metalness={.55} roughness={.2}/></mesh>{lower&&<><Line points={[[lower.value,-2,0],[lower.value,-.8,0]]} color="#ffd449" lineWidth={4}/><Endpoint3D q={lower} y={-2} color="#ffd449"/></>}{upper&&upper.value!==lower?.value&&<><Line points={[[upper.value,-2,0],[upper.value,-.8,0]]} color="#ffd449" lineWidth={4}/><Endpoint3D q={upper} y={-2} color="#ffd449"/></>}</group>}<Text position={[-5.8,2.55,0]} fontSize={.34} color="#ff8190">不等式 1</Text><Text position={[-5.8,.95,0]} fontSize={.34} color="#70f4b3">不等式 2</Text><Text position={[-5.8,-.25,0]} fontSize={.34} color="#ffe079">共通部分</Text>{s.empty&&<Text position={[0,-.8,0]} fontSize={.48} color="#ffd449">共通部分なし・解なし</Text>}<Camera preset={preset} zoom={zoom}/><OrbitControls makeDefault enablePan={false}/></Canvas>}

export default function App(){
 const[index,setIndex]=useState(0),[mode,setMode]=useState<'2d'|'3d'>('2d'),[lift,setLift]=useState<'diagonal'|'vertical'>('diagonal'),[step,setStep]=useState(0),[reveal,setReveal]=useState(false),[custom,setCustom]=useState(false),[inputs,setInputs]=useState<[string,string]>(['x > 1','x < 5']),[errors,setErrors]=useState(''),[preset,setPreset]=useState('angle'),[zoom,setZoom]=useState(1)
 ...
 <section className="stage panel"><div className="stagehead"><div><span className="status"><Sparkles size={14}/> VISUAL STAGE</span><h3>{step<3?steps[step]:'答えを確認しよう'}</h3></div><div className="stepdots">{steps.map((s,i)=><button key={s} className={i<=step?'active':''} onClick={()=>setStep(i)}>{i+1}</button>)}</div></div>
  {mode==='2d'&&<div className="lift-toggle" aria-label="立ち上がり方"><button className={lift==='diagonal'?'on':''} onClick={()=>setLift('diagonal')}>斜め上</button><button className={lift==='vertical'?'on':''} onClick={()=>setLift('vertical')}>垂直</button></div>}
  <div className={'visual '+mode}>{mode==='2d'?<NumberLine items={qs} step={step} lift={lift}/>:<Scene3D items={qs} preset={preset} zoom={zoom}/>}</div>
